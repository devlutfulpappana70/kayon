/* global jQuery:false */
/* global KAYON_STORAGE:false */

(function() {
	"use strict";

	function kayon_skin_post_header_init() {
	...
	kayon_add_filter( 'trx_addons_filter_skills_pie_options', function(opt) {
		opt.animateScale = false;
		return opt;
	} );

	kayon_add_filter( 'kayon_filter_select_container_selector', function(selector) {
	...
	kayon_add_filter( 'trx_addons_filter_slider_init_args', function( $param, $init ) {
    ...
    function skin_blogger_toggle_filter_add_element() {
        if ( jQuery('.content').find(".sc_style_toggle").length > 0 ) {
            jQuery(" .sc_style_toggle").each(function() {
                var toggle_title = KAYON_STORAGE['toggle_title'];
						...
						$share_items
							...
							.on('click', '.social_item[data-copy-link-url]', function(e) {
								...
								if ( url != '' ) {
									...
									var msg = $self.data('message') ? $self.data('message') : KAYON_STORAGE['msg_copied'];
									if ( msg ) {
	...
	// Audio
	function kayon_skin_audio_init() {
	...
	jQuery( window ).load(function() {
		...
		kayon_skin_post_header_init();

	});

	jQuery( document ).on(
		'action.ready_kayon', function() {
			...
			jQuery('.tribe-events-view').on( 'beforeAjaxComplete.tribeEvents beforeAjaxSuccess.tribeEvents beforeAjaxError.tribeEvents', kayon_tribe_events_after_ajax );
			function kayon_tribe_events_after_ajax( jqXHR, textStatus ) {
				setTimeout( function() {
					...
					btn.on( 'click', function(e) {
						...
						if ( $i_agree.length > 0 ) {
							if ( ! $i_agree.get(0).checked ) {
								form.append( '<div class="trx_addons_message_box trx_addons_message_box_error">' + KAYON_STORAGE['msg_i_agree_error'] + '</div>' );
						...
						var rez = kayon_form_validate(form, {
							...
							rules: [
								{
									field: 'EMAIL',
									min_length: {value: 7, message: KAYON_STORAGE['mc4wp_msg_email_min']},
									max_length: {value: 60, message: KAYON_STORAGE['mc4wp_msg_email_max']},
									mask: {
			...
			kayon_add_filter( 'kayon_filter_mobile_menu_close_timeout', function( timeout, $menu ) {
				return 1000;
			} );
            ...
            function kayon_skin_open_panel( id ) {
            ...
            jQuery( '.trx_addons_panel_menu_link:not(.panel_inited)' )
				...
				.on( 'click', function(e) {
					kayon_skin_open_panel( jQuery(this).attr('href').substring(1) );
					e.preventDefault();
					return false;
				});

            // Close panel
            function kayon_skin_close_panel() {
        ...
        jQuery( $search_wrap.each( function() {
            ...
            search_field.on('keyup', function(e) {
                if (e.keyCode === 27) {
                    ...
                    setTimeout(function () {
                        kayon_search_close($search_wrap_init);
    ...
    function kayon_search_close($search_wrap_init) {
	...
	jQuery( '.kayon_woocommerce_filters_button,.sidebar > .sidebar_close,.sidebar_overlay' ).on( 'click', function(e) {
