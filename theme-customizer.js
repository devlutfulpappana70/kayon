/* global kayon_color_schemes, kayon_dependencies, Color */
...
( function( api ) {
	...
	for (var i in kayon_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'kayon-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'kayon-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'kayon-vars' );
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + kayon_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( kayon_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + kayon_customizer_vars['msg_reload_title'] + '">' + kayon_customizer_vars['msg_reload'] + '</button>' );
			...
			if (kayon_customizer_vars && kayon_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( kayon_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', kayon_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							kayon_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( kayon_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'kayon_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var kayon_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					kayon_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									kayon_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								kayon_customizer_title_clicked( $self );
				...
				var kayon_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							kayon_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						kayon_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		kayon_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		kayon_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof kayon_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-kayon-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-kayon-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in kayon_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-kayon-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-kayon-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-kayon-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'kayon-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in kayon_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (kayon_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = kayon_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = kayon_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function kayon_customizer_get_field_value(fld) {
	...
	function kayon_customizer_check_dependencies(cont) {
		if ( typeof kayon_dependencies == 'undefined' || KAYON_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = kayon_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function kayon_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( kayon_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					kayon_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in kayon_theme_fonts) {
					for (rule in kayon_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							kayon_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function kayon_customizer_actions(e) {
	...
	function kayon_customizer_update_theme_vars(opt, value) {
		kayon_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function kayon_customizer_update_theme_fonts(opt, rule, value) {
		kayon_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function kayon_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= kayon_customizer_vars['max_load_fonts']; i++) {
	...
	function kayon_customizer_update_css() {
		...
		var vars = kayon_customizer_add_theme_vars( kayon_theme_vars );

		// Make styles and add its to the css
		css += kayon_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = kayon_customizer_add_theme_fonts( kayon_theme_fonts );

		// Make styles and add its to the css
		css += kayon_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in kayon_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( kayon_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in kayon_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in kayon_color_schemes[scheme].colors) {
				colors[i] = kayon_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = kayon_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-kayon-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function kayon_customizer_add_theme_colors(colors) {
		if (kayon_additional_colors) {
			var clr = '', v = '';
			for (var k in kayon_additional_colors) {
				v   = kayon_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + kayon_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + kayon_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + kayon_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function kayon_customizer_add_theme_vars(vars) {
		...
		if ( typeof vars['rad'] != 'undefined' ) {
			...
			rez['rad']      = kayon_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = kayon_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']      = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']        = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = kayon_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']    = kayon_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra'] = kayon_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']   = kayon_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']            = kayon_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']         = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']     = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']       = kayon_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']         = kayon_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']   = kayon_customizer_prepare_css_value( vars['sidebar_gap'] );
		}
		return rez;
	}

	// Add ed to css value
	function kayon_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function kayon_customizer_prepare_html_value(val) {
