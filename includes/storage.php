...
if ( ! function_exists( 'kayon_storage_get' ) ) {
	/**
	 ...
	 */
	function kayon_storage_get( $var_name, $default = '' ) {
		global $KAYON_STORAGE;
		return isset( $KAYON_STORAGE[ $var_name ] ) ? $KAYON_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'kayon_storage_set' ) ) {
	/**
	 ...
	 */
	function kayon_storage_set( $var_name, $value ) {
		global $KAYON_STORAGE;
		$KAYON_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'kayon_storage_empty' ) ) {
	/**
	 ...
	 */
	function kayon_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $KAYON_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $KAYON_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $KAYON_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'kayon_storage_isset' ) ) {
	/**
	 ...
	 */
	function kayon_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $KAYON_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $KAYON_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $KAYON_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'kayon_storage_unset' ) ) {
	/**
	 ...
	 */
	function kayon_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $KAYON_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $KAYON_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $KAYON_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'kayon_storage_inc' ) ) {
	/**
	 ...
	 */
	function kayon_storage_inc( $var_name, $value = 1 ) {
		global $KAYON_STORAGE;
		if ( empty( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = 0;
		}
		$KAYON_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'kayon_storage_concat' ) ) {
	/**
	 ...
	 */
	function kayon_storage_concat( $var_name, $value ) {
		global $KAYON_STORAGE;
		if ( empty( $KAYON_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'kayon_storage_get_array' ) ) {
	/**
	 ...
	 */
	function kayon_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $KAYON_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $KAYON_STORAGE[ $var_name ][ $key ] ) ? $KAYON_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'kayon_storage_set_array' ) ) {
	/**
	 ...
	 */
	function kayon_storage_set_array( $var_name, $key, $value ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$KAYON_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'kayon_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function kayon_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $KAYON_STORAGE[ $var_name ][ $key ] ) ) {
			$KAYON_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$KAYON_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$KAYON_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'kayon_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function kayon_storage_merge_array( $var_name, $key, $value ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$KAYON_STORAGE[ $var_name ] = array_merge( $KAYON_STORAGE[ $var_name ], $value );
		} else {
			$KAYON_STORAGE[ $var_name ][ $key ] = array_merge( $KAYON_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'kayon_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function kayon_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			kayon_array_insert_after( $KAYON_STORAGE[ $var_name ], $after, $key );
		} else {
			kayon_array_insert_after( $KAYON_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'kayon_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function kayon_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			kayon_array_insert_before( $KAYON_STORAGE[ $var_name ], $before, $key );
		} else {
			kayon_array_insert_before( $KAYON_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'kayon_storage_push_array' ) ) {
	/**
	 ...
	 */
	function kayon_storage_push_array( $var_name, $key, $value ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $KAYON_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $KAYON_STORAGE[ $var_name ][ $key ] ) ) {
				$KAYON_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $KAYON_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'kayon_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function kayon_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $KAYON_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $KAYON_STORAGE[ $var_name ] ) && is_array( $KAYON_STORAGE[ $var_name ] ) && count( $KAYON_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $KAYON_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $KAYON_STORAGE[ $var_name ][ $key ] ) && is_array( $KAYON_STORAGE[ $var_name ][ $key ] ) && count( $KAYON_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $KAYON_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'kayon_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function kayon_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( empty( $KAYON_STORAGE[ $var_name ][ $key ] ) ) {
			$KAYON_STORAGE[ $var_name ][ $key ] = 0;
		}
		$KAYON_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'kayon_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function kayon_storage_concat_array( $var_name, $key, $value ) {
		global $KAYON_STORAGE;
		if ( ! isset( $KAYON_STORAGE[ $var_name ] ) ) {
			$KAYON_STORAGE[ $var_name ] = array();
		}
		if ( empty( $KAYON_STORAGE[ $var_name ][ $key ] ) ) {
			$KAYON_STORAGE[ $var_name ][ $key ] = '';
		}
		$KAYON_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'kayon_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function kayon_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $KAYON_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $KAYON_STORAGE[ $var_name ] ) ? $KAYON_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $KAYON_STORAGE[ $var_name ] ) ? $KAYON_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'kayon_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function kayon_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $KAYON_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $KAYON_STORAGE[ $var_name ]->$prop ) ? $KAYON_STORAGE[ $var_name ]->$prop : $default;
	}
}
