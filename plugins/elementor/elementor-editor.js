...
jQuery( document ).ready(
	function() {
		...
		kayon_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="kayon_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.kayon_post_editor' );
						if ( link.length > 0 ) {
							kayon_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.kayon_post_editor', function(e) {
						kayon_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('kayon_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function kayon_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
