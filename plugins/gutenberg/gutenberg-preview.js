...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			kayon_create_observer( 'kayon-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				kayon_gutenberg_editor_init();
			} );
		} else {
		 	kayon_gutenberg_editor_init();
		}
	...
	function kayon_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'kayon_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'kayon_inited' ) ) {
	...
	// Init on page load
	function kayon_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = kayon_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + KAYON_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + KAYON_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in KAYON_STORAGE['override_classes'] ) {
				$obj.addClass( KAYON_STORAGE['override_classes'][i].replace( '%s', KAYON_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( kayon_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && KAYON_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'kayon_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			kayon_remove_observer( 'kayon-check-editor-styles-wrapper' );
			kayon_create_observer( 'kayon-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'kayon_inited' );
	}
} );
