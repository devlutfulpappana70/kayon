<div class="front_page_section front_page_section_features<?php
	$kayon_scheme = kayon_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $kayon_scheme ) && ! kayon_is_inherit( $kayon_scheme ) ) {
		echo ' scheme_' . esc_attr( $kayon_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( kayon_get_theme_option( 'front_page_features_paddings' ) );
	if ( kayon_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$kayon_css      = '';
		$kayon_bg_image = kayon_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$kayon_anchor_icon = kayon_get_theme_option( 'front_page_features_anchor_icon' );
	$kayon_anchor_text = kayon_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( kayon_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$kayon_css      = '';
			$kayon_bg_mask  = kayon_get_theme_option( 'front_page_features_bg_mask' );
			$kayon_bg_color_type = kayon_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $kayon_bg_color_type ) {
				$kayon_bg_color = kayon_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$kayon_caption = kayon_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $kayon_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $kayon_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $kayon_caption, 'kayon_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$kayon_description = kayon_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $kayon_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $kayon_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $kayon_description ), 'kayon_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! kayon_exists_trx_addons() ) {
						...
						kayon_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
