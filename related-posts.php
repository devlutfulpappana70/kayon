...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $kayon_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	kayon_show_post_featured(
		array(
			'thumb_size' => apply_filters( 'kayon_filter_related_thumb_size', kayon_get_thumb_size( (int) kayon_get_theme_option( 'related_posts' ) == 1 ? 'huge' : 'big' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<span class="post_date"><a href="<?php echo esc_url( $kayon_link ); ?>"><?php echo wp_kses_data( kayon_get_date() ); ?></a></span>
			<?php
		}
		?>
	</div>
